...
static void AWGNAddNoise(float es_ovr_n0, long channel_len, float *in_array, float *out_array);
...
WORD AWGNChannel(WORD *inBuffer, WORD *inSize, WORD *outBuffer, WORD *outSize, BYTE *BlockObjStruct)
{
	...
	SWORD outputSignalLevel;
	WORD inputSignalLevel;
	float inputFloat;
	float outputFloat;
	AWGN_CHANNEL_OBJ_STRUCT *AWGNChannelObjStruct = (AWGN_CHANNEL_OBJ_STRUCT *)BlockObjStruct;
	...
	switch (AWGNChannelObjStruct->currentState)
	{
		case (AWGN_NORMAL_STATE):
			while (inputSize-- > 0)
			{
				inputSignalLevel = *inBuffer++;

				//
				// Normalize to a floating value between -1 and 1
				//
				inputFloat = (float)((SWORD)inputSignalLevel)
							/ (float)AWGN_SYMBOL_NORMALIZATION;

				AWGNAddNoise((float)(SNRdB), 1,  &inputFloat, &outputFloat);
				...
				outputSignalLevel = (WORD)(outputFloat * AWGN_SYMBOL_NORMALIZATION);
				

				*outBuffer++ = (WORD)outputSignalLevel;
				outputSize++;				
			}
			break;
		default:
